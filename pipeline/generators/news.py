...
def generate_news_region(
    ...
    Wrapper that delegates to generate_news_sections for the single region.
    Kept as a separate function so generate.py can call it per-region
    and get independent status.set_step() tracking for each region.
    """
    result = generate_news_sections({region: rows}, adapter=adapter)
...
def generate_news_sections(news_rows: dict[str, list], adapter=None) -> dict[str, list[dict[str, Any]]]:
    ...
    Returns:
        dict mapping region key to list of summary dicts.
    """
    ...
    for region, rows in news_rows.items():
        ...
        articles = []
        for row in rows:
            # Handle both dict-like (sqlite3.Row) and plain dict row objects
            if hasattr(row, "get"):
                # Plain dict
                title = row.get("title", "")
                ...
                # sqlite3.Row — use dict() conversion then .get
                d = dict(row)
                title = d.get("title", "")
                source = d.get("source", "")
                ...
                title = source = region_val = category = full_text = excerpt = url = ""
